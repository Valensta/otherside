...
public class SpyGlass : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IPointerClickHandler
{
    ...
    {
    //    if (_enabled && disabled_by_drag_button != _enabled) Debug.Log("Disabled by drag button " + _enabled + "\n");
    ...
    public void PointSpyglass(Vector2 new_pos,float window)
    {
        bool in_window = true;
        Vector2 me = my_transform.position;
        Vector2 dist = new_pos - me;
      //  Debug.Log(me + " to " + dist + " -> x = " + map_x_size + " y = " + map_y_size + "\n");
        if (Mathf.Abs(dist.x) < map_x_size * window && Mathf.Abs(dist.y) < map_y_size * window) return;
    ...
    public void PointSpyglass(Vector2 new_pos)
    {        
        PointSpyglass(new_pos, 1f);   
    }
