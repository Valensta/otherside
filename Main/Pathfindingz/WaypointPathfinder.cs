...
*/
    ...
    public List<WaypointNodelet> FindPath(Vector3 startPos, Vector3 endPos)
	{//print ("findpath");
       ...
       // CheckEndPosition(endPos);
        ...
        if (startNode != null && endNode != null)
		{
            ...
            if (freeSpot && SearchMethod == WaypointMethod.ExactPosition)
            {
                ...
                {
                    if (Vector3.Distance(returnPath[returnPath.Count - 1].position, returnPath[returnPath.Count - 3].position) < Vector3.Distance(returnPath[returnPath.Count - 3].position, returnPath[returnPath.Count - 2].position) && freeSpot)
                    {
    ...
    private WaypointListNode FindClosestNode(Vector3 pos)
    {
        ...
        if (ID > -1)
        {
            WaypointListNode wp = new WaypointListNode(Map[ID].position, Map[ID].ID, Map[ID].Iii, null, Map[ID].neighbors);
            return wp;
