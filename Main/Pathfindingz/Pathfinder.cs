...
public class Pathfinder : MonoBehaviour 
{
    ...
    private void CreateMap()
    {
        ...
        for (int i = 0; i < height; i++)
        {
            for (int j = 0; j < width; j++)
            {
                ...
                RaycastHit[] hit; 
			//	print ("x " + x + " y " + HighestPoint + " z " + y + " dist " + dist);
    ...
    public void InsertInQueue(Vector3 startPos, Vector3 endPos, Action<List<WaypointNodelet>> listMethod)
    {
    ...
    IEnumerator PathHandler(Vector3 startPos, Vector3 endPos, Action<List<WaypointNodelet>> listMethod)
    {
        yield return StartCoroutine(SinglePath(startPos, endPos, listMethod));
    }

    IEnumerator SinglePath(Vector3 startPos, Vector3 endPos, Action<List<WaypointNodelet>> listMethod)
    {
        FindPath(startPos, endPos, listMethod);
        yield return null;
    }

    public void FindPath(Vector3 startPos, Vector3 endPos, Action<List<WaypointNodelet>> listMethod)
    {
		...
		//	print ("have a start node\n");
            ...
            {
                returnPath.Add(new WaypointNodelet(new Vector3(currentNode.xCoord, currentNode.yCoord, currentNode.zCoord), currentNode.ID));
            ...
            if (returnPath.Count > 2 && endPosValid)
            {
                ...
                if (Vector3.Distance(returnPath[returnPath.Count - 1].position, returnPath[returnPath.Count - 3].position) < Vector3.Distance(returnPath[returnPath.Count - 3].position, returnPath[returnPath.Count - 2].position))
                {
