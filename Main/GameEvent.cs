...
public class GameEvent : MonoBehaviour {
	...
	IEnumerator WaitForStart(){
     //   Debug.Log("Running Coroutine WaitForStart " + this.gameObject.name + "\n");
   ...
   //         Debug.Log("Initializing trigger " + t.condition + "\n");
		...
		while (togo > 0) {
            if (Central.Instance == null) yield return StartCoroutine(CoroutineUtil.WaitForRealSeconds(check_interval));
            if (is_intralevel && Central.Instance.getState() != GameState.LevelList) yield return StartCoroutine(CoroutineUtil.WaitForRealSeconds(check_interval));
            if (!is_intralevel && Central.Instance.getState() != GameState.InGame) yield return StartCoroutine(CoroutineUtil.WaitForRealSeconds(check_interval));

            if (kill_trigger != null && kill_trigger.condition != Condition.Null && 
                !(kill_trigger.condition == Condition.TIME && kill_trigger.number == 0) &&  //what it falls back to if you forget to set it
                kill_trigger.Check())
            {

                Debug.Log("Killing GameEvent " + this.gameObject.name + "\n");
                is_waiting = false;
                onEventComplete(my_name, reward);
                yield return null;
            }
