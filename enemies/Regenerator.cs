...
public class Regenerator : Modifier
{
    ...
    Firearm my_firearm;
    int level;
    EffectType effect_type = EffectType.Null; //specify if this is used by DOT
    ...
    void HealMe()
    {
        ...
        {
         //   Debug.Log(effect_type  + " Regenerator doing stuff " + rate + " " + heal_duration + " " + Time.realtimeSinceStartup + "\n");
            if (effect_type == EffectType.DOT && my_firearm != null) my_hitme.MassMe(stats, my_firearm, effect_type, level);
             else my_hitme.MassMe(stats);
