...
public class WaypointAI : Pathfinding {
    ...
    private void MoveMethod()
    {
        if (Path.Count > 0)
        {
            ...
            if (Vector3.Distance(transform.position - Vector3.up, Path[0].position) < 1F)
            {
                Path.RemoveAt(0);
            }
        }
    ...
    private void DrawPath()
    {
        if (Path.Count > 0)
        {
            ...
            {
                lineRenderer.SetPosition(i, Path[i].position + Vector3.up);
            }
        }
        else
        {
