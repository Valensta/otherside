...
public class Pathfinder : MonoBehaviour 
{
    ...
    private void CreateMap()
    {
        ...
        for (int i = 0; i < height; i++)
        {
            for (int j = 0; j < width; j++)
            {
                ...
                Map[j, i] = new Node(j, i, y, ID, x, true); //walkable tile!
    ...
    IEnumerator PathHandler(Vector2 startPos, Vector2 endPos, Action<List<WaypointNodelet>> listMethod)
    {
        yield return StartCoroutine(SinglePath(startPos, endPos, listMethod));
    }

    IEnumerator SinglePath(Vector2 startPos, Vector2 endPos, Action<List<WaypointNodelet>> listMethod)
    {
        FindPath(startPos, endPos, listMethod);
        yield return null;
    }

    public void FindPath(Vector2 startPos, Vector2 endPos, Action<List<WaypointNodelet>> listMethod)
    {
        print("Using this findpath\n");
        ...
        if (startNode != null)
        {
		//	print ("have a start node\n");
            if (endNode == null)
            {
			//	print ("no obvious end node\n");
                endPosValid = false;
                FindEndNode(endPos);
                if (endNode == null)
                {
				//	print("No end node\n");
                    ...
                    maxSearchRounds = 0;
                    listMethod.Invoke(new List<WaypointNodelet>());
                    return;
                }
            }
            ...
            {
                returnPath.Add(new WaypointNodelet(new Vector2(currentNode.xCoord, currentNode.yCoord), currentNode.ID));
            ...
            if (returnPath.Count > 2 && endPosValid)
            {
                ...
                if (Vector2.Distance(returnPath[returnPath.Count - 1].position, returnPath[returnPath.Count - 3].position) < Vector2.Distance(returnPath[returnPath.Count - 3].position, returnPath[returnPath.Count - 2].position))
                {
                    returnPath.RemoveAt(returnPath.Count - 2);
                }
                if (Vector2.Distance(returnPath[1].position, startPos) < Vector2.Distance(returnPath[0].position, returnPath[1].position))
                {
    ...
    private void SetStartAndEndNode(Vector2 start, Vector2 end)
    {
        startNode = FindClosestNode(start);
        endNode = FindClosestNode(end);
    }

    public bool IsTheClosestNodeWalkable(Vector2 pos)
    {
    ...
    {      
        ...
        if (n.walkable)
        {
            ...
            return new Node(x, y, n.yCoord, n.ID, n.xCoord, n.walkable);
        }
        else
        {
            ...
            for (int i = y - 1; i < y + 2; i++)
            {
                for (int j = x - 1; j < x + 2; j++)
                {
                    ...
                    if (i > -1 && i < Map.GetLength(1) && j > -1 && j < Map.GetLength(0))
                    {
                        if (Map[j, i].walkable)
                        {
                            return new Node(j, i, Map[j, i].yCoord, Map[j, i].ID, Map[j, i].xCoord, Map[j, i].walkable);
                        }
                    }
                }
            }
    ...
    private void FindEndNode(Vector2 pos)
    {       
        int x = (MapStartPosition.x < 0F) ? Mathf.FloorToInt(((pos.x + Mathf.Abs(MapStartPosition.x)) / Tilesize)) : Mathf.FloorToInt((pos.x - MapStartPosition.x) / Tilesize);
        int y = (MapStartPosition.y < 0F) ? Mathf.FloorToInt(((pos.y + Mathf.Abs(MapStartPosition.y)) / Tilesize)) : Mathf.FloorToInt((pos.y - MapStartPosition.y) / Tilesize);
        ...
        if (walkableNodes.Count > 0) //If we found some walkable tiles we will then return the nearest
        {
            ...
            endNode = new Node(n.x, n.y, n.yCoord, n.ID, n.xCoord, n.walkable);
        }
    }
                        ...
                        if (Map[j, i].walkable)
                        {
                            ...
                            if (!OnClosedList(Map[j, i].ID))
                            {
                                //Check if we can move up or jump down!
                                    ...
                                    {
                                        Node addNode = new Node(Map[j, i].x, Map[j, i].y, Map[j, i].yCoord, Map[j, i].ID, Map[j, i].xCoord, Map[j, i].walkable, currentNode);
                ...
                for (int j = 0; j < Map.GetLength(0); j++)
                {
                    ...
                    for (int y = i - 1; y < i + 2; y++)
                    {
                        for (int x = j - 1; x < j + 2; x++)
                        {
                            ...
                            Vector2 start = new Vector2(Map[j, i].xCoord, Map[j, i].yCoord + 0.1f);
                            Vector2 end = new Vector2(Map[x, y].xCoord, Map[x, y].yCoord + 0.1f);
    ...
    {
        Vector2 startPos = new Vector2(b.min.x, 0);
