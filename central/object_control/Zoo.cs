...
public class Zoo : MonoBehaviour {
    ...
    void Awake()
    {
        ...
        Instance = this;
        zoo_id = this.gameObject.transform.GetInstanceID();
    }
...
//		Debug.Log("Done with smarrt init\n");
	...
	public GameObject getObject(string name, bool active){
        ...
        {
            Debug.LogError("Pool could not get object " + name + ": " + e.Message);
        }
        return null;
	}
	...
	GameObject GetFromPool(ref Pool p, bool active){
        ...
        {
            if (p.pool[i] == null || p.pool[i].activeSelf) continue;
           
            if (p.require_explicit_return && p.pool[i].transform.parent.GetInstanceID() != zoo_id) continue;
            o = p.pool[i];
            break;
        }
...
//		o.transform.parent = this.transform;
	...
	public void returnAll()
    {
        ...
        {
            if (p.non_refundable || p.require_explicit_return)
            {
   ...
   //     Debug.Log("Returning " + o.name + "\n");
    ...
    //   Debug.Log("Adding " + name + "\n");
        while (count > 0) {
            ...
            UnityEngine.Object r = Resources.Load(name, typeof(GameObject));
