...
public class RegularTrigger : Trigger {
   ...
   //     Debug.Log("Initializing trigger " + this.text + " " + this.condition + "\n");
	    ...
	    if (condition == Condition.Selected) Island_Button.onSelected += onSelected;
        ...
        if (condition == Condition.CanUpgrade)Rune.onCanUpgrade += onCanUpgrade;
    ...
    public override bool CheckConditions()
    {
        ...
        switch (condition)
        {
            ...
            case Condition.TIMEInterval:
                ...
                {
                    Debug.Log("Cannot find peripheral\n");
                    return false;
                }
            ...
            case Condition.UpgradeSkill:
                return selected;
            case Condition.CanUpgrade:
                return selected;
            case Condition.Resume:
                if (!Peripheral.Instance.isPaused())
                {
                    Debug.Log("Got resume\n");
                    return !Peripheral.Instance.isPaused();
                }
                return false;
        case Condition.Lavaburn:
                return selected;
            case Condition.HaveCity:
                return Peripheral.Instance.have_cities;
         ...
         //       Debug.Log("Wave ended " + number + "\n");
          ...
          //      Debug.Log("Ok you won\n");
                return false;
            case Condition.MaxLvlReached: //this will never work for regular events, because peripheral waits for overseer.ingame_finished == true before declaring that the level is won
                if (Central.Instance.getState() == GameState.LevelList && (Central.Instance.level_list.getActualMaxLvl()) >= number)
                {
                    //Debug.Log("Ok Max level reached\n");
                    return true;
                }
                return false;
