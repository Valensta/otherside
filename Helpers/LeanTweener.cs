	...
	public void Init () {
     //   Debug.Log("Initializing " + target.gameObject.name + "\n");
		...
		case TweenType.Scale:			
			...
			l = LeanTween.scale(target, vector, time);			
			break;
		case TweenType.Rotate:
			l = LeanTween.rotateZ(target, value, time);
			break;
		case TweenType.ColorChange:
			l = LeanTween.color(target, my_color, time);
			break;
		default:
			return;	
		}
    ...
    void OnEnable()
    {
        switch (preset)
        {
            ...
            case LeanTweenerPreset.UIBlip: //quick blip for a ui item like health or dream status
                ...
                pingpong = 1;
                break;
            case LeanTweenerPreset.UI3Blips: //quick blip for a ui item like health or dream status
                duration = 0.9f;
                time = 0.3f;
                leantweentype = LeanTweenType.easeInQuad;
                delay = 0.1f;
